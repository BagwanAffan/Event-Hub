...
export default function HowItWorksSection() {
  ...
  return (
    <section className="py-20 lg:py-32 bg-slate-50 dark:bg-[#151515]/50 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-8 relative z-10">
            {steps.map((step, i) => (
              <div key={i} className="flex flex-col items-center text-center">
                <div className="w-16 h-16 rounded-full bg-white dark:bg-[#181818] border-4 border-slate-50 dark:border-white/[0.08] shadow-xl flex items-center justify-center mb-4 relative">
                ...
                <h3 className="font-semibold text-slate-900 dark:text-[#F5F5F5] mb-1">{step.title}</h3>
                <p className="text-sm text-slate-500 dark:text-[#9CA3AF]">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
